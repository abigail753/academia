...
public interface ExamenRepository extends JpaRepository<ExamenEntity, Long> {
        ...
        @Query(value = "SELECT e.* FROM examen e INNER JOIN calificacion c ON e.id = c.id_examen WHERE e.id = :id_examen AND c.id_usuario = :id_usuario", nativeQuery = true)
        Optional<ExamenEntity> findExamenById(@Param("id_examen") Long id_examen, @Param("id_usuario") Long id_usuario);
